...
with open(f"{data_dir}/{username}_keyFiles.txt") as f:
    # Output results
    if key_files_data:
        for repo_data in key_files_data:
            f.write(f"\nRepository: {repo_data['Repository']}")
            for file in repo_data['Key Files']:
                f.write(f"\n  File: {file['File Name']}, Size: {file['Size (KB)']} KB, Path: {file['Path']}")
    else:
        f.write("No key files found.")
